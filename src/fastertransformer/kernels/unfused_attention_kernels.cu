                               ...
                               const float qk_scale)
{
    ...
    // const int qi = blockIdx.y;  // q index.
    const int qi = threadIdx.x;  // q index.
    // const int ki = threadIdx.x;  // k index.
    ...
    float local_max = -1e20f;
    float local_sum = 0.0;
    int qk_offset = hi * q_length * k_length + qi * k_length;
    int mask_offset = qi * k_length;
    for (int ki = 0; ki < k_length; ki++){
        float qk_val  = static_cast<float>(qk[qk_offset + ki]);
        float qk_bias = 0.0f;

        float mask_val    = static_cast<float>(ldg(&attn_mask[mask_offset + ki]));
        qk_bias += mask_val * -10000.0f;

        data[ki]  = qk_val + qk_bias;
        local_max = fmax(local_max, data[ki]);
    }

    for (int ki = 0; ki < k_length; ki++){
        data[ki] = __expf(data[ki] - local_max);
        local_sum += data[ki];
    }

    s_mean = __fdividef(1.0f, local_sum);

    for (int ki = 0; ki < k_length; ki++){
        attn_score[qk_offset + ki] = (T)(data[ki] * s_mean);
    }
    ...
    //     int   mask_offset = qi * k_length + ki;
    //     float mask_val    = static_cast<float>(ldg(&attn_mask[mask_offset]));
    //     qk_bias += mask_val * -10000.0f;

    //     data[0]   = qk_scale * qk_val + qk_bias;
    //     local_max = fmax(local_max, data[0]);
    // } else {
    //     data[0] = 0;
    // }
    ...
    // if (ki < k_length){
    //     attn_score[qk_offset + ki] = (T)(data[0] * s_mean);
    // }
}
...
#pragma unroll
            for (int j = 0; j < Q_ITEMS; j++) {
                ...
                mask_val[j] = hmul2<T2>(mask_val[j], NEG_INFTY);
            }

#pragma unroll
            for (int j = 0; j < Q_ITEMS; j++) {
                // T2 val = hadd2<T2>(hmul2<T2>(qk_scale, qk[j]), mask_val[j]);
                T2 val = hadd2<T2>(qk[j], mask_val[j]);
                if (linear_bias_slopes != nullptr) {
...
void invokeMaskedSoftmax(MaskedSoftmaxParam<T, T_IN>& param, cudaStream_t stream)
{
    ...
    // dim3 grid(param.num_heads, (param.q_length + 31) / 32 * 32);
    dim3 grid(param.num_heads);
    ...
    bool is_half2 = false;
    // size T:2, sizeof(T_IN): 2, is_half2: 1
    // dim3 block((param.k_length / (is_half2 ? 2 : 1) + 31) / 32 * 32);
    // dim3 block(max((param.k_length + 31) / 32 * 32), 32);
    dim3 block(param.q_length);
                                         ...
                                         const int    int8_mode)
{
    ...
    const int hidden_num = head_num * size_per_head;
    const int seq_head_size = size_per_head * seq_len;

    // const int src_offset_base = src_batch_id * seq_len * head_num * size_per_head + src_seq_id * size_per_head;
    const int src_offset_base = src_batch_id * seq_len * hidden_num + src_seq_id * hidden_num;
    // const int dst_offset_base = dst_seq_id * head_num * size_per_head;
    const int dst_offset_base = dst_seq_id * hidden_num / seq_len;
    ...
    for (int idx = threadIdx.x; idx < hidden_num; idx += blockDim.x) {
        // const int head_id   = idx / size_per_head;
        // const int hidden_id = idx % size_per_head;

        const int dst_head_id = idx / seq_head_size;
        const int token_id    = idx % seq_len;
        const int hidden_id   = (idx % seq_head_size) / seq_len;

        // const T   src_elem  = ldg(&src[src_offset_base + head_id * seq_len * size_per_head + hidden_id]);
        const T   src_elem  = ldg(&src[src_offset_base + idx]);
        if (int8_mode == 2) {
            ...
            dst[token_id * hidden_num + dst_offset_base + dst_head_id * size_per_head + hidden_id] = src_elem;
        }
    }
}
                                              ...
                                              cudaStream_t stream)
{
    ...
    if (is_half2) {
        ...
        transpose_remove_padding<T><<<valid_word_num, block_size, 0, stream>>>(
            (T*)src, (T*)dst, batch_size, seq_len, head_num, size_per_head, mask_offset, scale, int8_mode);
    }
    else {
                                                   ...
                                                   const int    int8_mode)
{
    ...
    for (int index = blockDim.x * blockIdx.x + threadIdx.x; index < token_num * 3 * n;
        ...
        const int qkv_id  = (index % (3 * size_per_head)) / size_per_head;
        // const int head_id = (index % n) / size_per_head;
        const int head_id = (index % (3 * n)) / (3 * size_per_head);
        ...
        qkv_ptr[qkv_id][target_batch_id * head_num * seq_len * size_per_head + seq_id * head_num * size_per_head + head_id * size_per_head + size_id] = val;
    }
}
                                                   ...
                                                   const bool neox_rotary_style)
{
    ...
    const int tidx_vec = tidx * vec_size;
    const bool is_masked = tidx_vec >= size_per_head;
    // NOTE: blockIdx.x < batch_size * param.max_prefix_prompt_length really handles prefix prompts
    // false
    if (PREFIX_PROMPT && token_idx < 0) {
        const int prompt_batch_idx = blockIdx.x / param.max_prefix_prompt_length;
        ...
        if (prompt_seq_idx < prompt_length) {
            const int dest_kv_idx = prompt_batch_idx * size_per_head * total_seq_len * head_num
                                    ...
                                    + tidx_vec;
            const int prefix_kv_idx =
                size_per_head * prompt_length * head_idx + size_per_head * prompt_seq_idx + tidx_vec;
    ...
    const int prefix_prompt_length = PREFIX_PROMPT ? param.d_prefix_prompt_lengths[batch_idx] : 0;
    // const int hidden_idx           = head_idx * size_per_head + tidx_vec;
    const int hidden_idx           = head_idx * size_per_head * 3 + tidx_vec;
    const int n                    = head_num * size_per_head;
    ...
    // Note: if !PREFIX_PROMPT, max_pp_len = 0, so it's no-op
    const int dst_kv_seq_idx = seq_idx + prefix_prompt_length;
    // add pos id and block pos id, add by wuziyang
    const int position_id = seq_len > 1 ? seq_idx : param.position_id;
    const int block_position_id = seq_len > 1 ? (seq_idx + 1 == seq_len ? 1 : 0) : param.block_position_id;
    ...
    // const int src_k_idx = token_idx * 3 * n + hidden_idx + n;
    // const int src_v_idx = token_idx * 3 * n + hidden_idx + 2 * n;
    const int src_k_idx = token_idx * 3 * n + hidden_idx + size_per_head;
    const int src_v_idx = token_idx * 3 * n + hidden_idx + 2 * size_per_head;
    ...
    if (!is_masked) {
        ...
        // k_bias = *reinterpret_cast<const Vec_t*>(&qkv_bias[hidden_idx + n]);
        // v_bias = *reinterpret_cast<const Vec_t*>(&qkv_bias[hidden_idx + 2 * n]);
        k_bias = *reinterpret_cast<const Vec_t*>(&qkv_bias[hidden_idx + size_per_head]);
        v_bias = *reinterpret_cast<const Vec_t*>(&qkv_bias[hidden_idx + 2 * size_per_head]);
    }
    ...
    if (!neox_rotary_style) {
        ...
        // const bool do_rotary = !is_masked && vec_size * tidx < rotary_embedding_dim;
        // const bool do_rotary = tidx_vec >= rotary_embedding_dim;
        // const bool do_rotary = false;
        const bool do_rotary = true;

        T* q_smem = reinterpret_cast<T*>(smem_);
        T* k_smem = q_smem + rotary_embedding_dim;
        T* q2_smem = q_smem + rotary_embedding_dim * 2;
        T* k2_smem = q_smem + rotary_embedding_dim * 3;

        // rotary_embedding_dim: 64, half_rotary_dim: 32
        const int half_rotary_dim = rotary_embedding_dim / 2;
        const int half_idx        = tidx_vec / half_rotary_dim;
        const int intra_half_idx  = tidx_vec % half_rotary_dim;
        const int smem_pitch      = half_rotary_dim;  // TODO: adjust for bank conflicts?

        if (do_rotary) {
            // *reinterpret_cast<Vec_t*>(q_smem + half_idx * smem_pitch + intra_half_idx) = q;
            // *reinterpret_cast<Vec_t*>(k_smem + half_idx * smem_pitch + intra_half_idx) = k;
            if (tidx_vec < rotary_embedding_dim){
                *reinterpret_cast<Vec_t*>(q_smem + tidx_vec) = q;
                *reinterpret_cast<Vec_t*>(k_smem + tidx_vec) = k;
            } else {
                *reinterpret_cast<Vec_t*>(q2_smem + tidx_vec - rotary_embedding_dim) = q;
                *reinterpret_cast<Vec_t*>(k2_smem + tidx_vec - rotary_embedding_dim) = k;
            }
        }
        ...
        // rotary needs two split, [0, 63], [64, 127], use different posid
        if (do_rotary) {
            if (tidx_vec < rotary_embedding_dim){
                const int     transpose_idx = half_idx * (half_rotary_dim / 2) + intra_half_idx / 2;
                mmha::vec_from_smem_transpose(q, q_smem, transpose_idx, smem_pitch);
                ...
                // mmha::apply_rotary_embedding(q, k, transpose_idx / tidx_factor, rotary_embedding_dim, dst_kv_seq_idx);
                mmha::apply_rotary_embedding(q, k, transpose_idx / tidx_factor, rotary_embedding_dim, position_id);

                mmha::write_smem_transpose(q, q_smem, transpose_idx, smem_pitch);
                mmha::write_smem_transpose(k, k_smem, transpose_idx, smem_pitch);
            } else {
                // 原计算方法在id>=64时,会使用32-96的id计算rotary,实际应该是64-127
                const int     transpose_idx = (tidx_vec - rotary_embedding_dim) / 2;
                mmha::vec_from_smem_transpose(q, q2_smem, transpose_idx, smem_pitch);
                mmha::vec_from_smem_transpose(k, k2_smem, transpose_idx, smem_pitch);

                // mmha::apply_rotary_embedding(q, k, transpose_idx / tidx_factor, rotary_embedding_dim, dst_kv_seq_idx);
                mmha::apply_rotary_embedding(q, k, transpose_idx / tidx_factor, rotary_embedding_dim, block_position_id);

                mmha::write_smem_transpose(q, q2_smem, transpose_idx, smem_pitch);
                mmha::write_smem_transpose(k, k2_smem, transpose_idx, smem_pitch);
            }
        }
        ...
        if (do_rotary) {
            ...
            // k = *reinterpret_cast<Vec_t*>(k_smem + half_idx * smem_pitch + intra_half_idx);
            if (tidx_vec < rotary_embedding_dim){
                q = *reinterpret_cast<Vec_t*>(q_smem + tidx_vec);
                k = *reinterpret_cast<Vec_t*>(k_smem + tidx_vec);
            } else {
                q = *reinterpret_cast<Vec_t*>(q2_smem + tidx_vec - rotary_embedding_dim);
                k = *reinterpret_cast<Vec_t*>(k2_smem + tidx_vec - rotary_embedding_dim);
            }
        }
    }

    // 根据python版本，query_layer = query_layer / (math.sqrt(hidden_size) * query_key_layer_scaling_coeff)
    // python版本存在冗余计算，需要删除query_key_layer_scaling_coeff部分
    // T hidd = 0.0883883476483184 / float(layer_id + 1);
    T hidd = 0.0883883476483184;
    T* q_split = reinterpret_cast<T*>(&q);
    *(q_split) = *(q_split) * hidd;
    *(q_split + 1) = *(q_split + 1) * hidd;
    ...
    //     const int dest_q_idx = batch_idx * size_per_head * seq_len * head_num + head_idx * size_per_head * seq_len
    //                            + seq_idx * size_per_head + tidx_vec;

    //     const int dest_kv_idx = batch_idx * size_per_head * seq_len * head_num + head_idx * size_per_head * seq_len
    //                            + dst_kv_seq_idx * size_per_head + tidx_vec;

    // const int dst_hidden_idx           = head_idx * size_per_head + tidx_vec;
    // const int dst_idx                  = token_idx * n + dst_hidden_idx;

    // 对于第一次，直接使用seq_len就可以了，后面的再说
    const int dst_hidden_idx          = token_idx * size_per_head + tidx_vec;
    // const int dst_idx = head_idx * size_per_head * seq_len + dst_hidden_idx;


    if (!is_masked) {
        const int dst_idx = head_idx * size_per_head * seq_len + dst_hidden_idx;
        // const int dst_idx = head_idx * size_per_head + size_per_head * seq_len + tidx_vec;
        *reinterpret_cast<Vec_t*>(&q_buf[dst_idx]) = q;
        *reinterpret_cast<Vec_t*>(&k_buf[dst_idx]) = k;
        *reinterpret_cast<Vec_t*>(&v_buf[dst_idx]) = v;
    }
}
                                    ...
                                    cudaStream_t                     stream)
{
    ...
    // if (seq_len == 2){
    //     std::cout << "seqlen:" << seq_len << std::endl;
    // }
    // true
    if (rotary_embedding_dim == 0 && param.max_prefix_prompt_length == 0) {
        ...
        size_t smem_size = neox_rotary_style ? 4 * rotary_embedding_dim * sizeof(T) : 0;
...
template<typename T>
__global__ void transpose_4d_batch_major_kv_cache(
    T* d_dst, T* d_src, const int head_num, const int size_per_head, const int seq_len, const int cache_idx, const int max_seq_len)
{
    const int token_id = blockIdx.x;
    const int batch_id = blockIdx.y;
    const int head_id  = blockIdx.z;

    const int size_per_head_zip = size_per_head / (sizeof(uint4) / sizeof(T));

    // 16 byte loads will handle "x" dimension
    auto data_src = reinterpret_cast<const uint4*>(d_src + batch_id * head_num * size_per_head * seq_len);

    const int dst_index = batch_id * head_num * size_per_head + cache_idx * head_num * size_per_head;
    auto data_dst = reinterpret_cast<uint4*>(d_dst + dst_index);

    // idx is over output dimension L * size_per_head / x for values
    const int src_idx = (token_id * head_num * size_per_head_zip) + (head_id * size_per_head_zip) + threadIdx.x;
    int dst_idx;
    if (seq_len > 1) {
        dst_idx = ((token_id * head_num + head_id) % seq_len) * head_num * size_per_head_zip + ((token_id * head_num + head_id) / seq_len) * size_per_head_zip + threadIdx.x;
    } else {
        dst_idx = (head_id * size_per_head_zip) + threadIdx.x;
    }
    ...
    int     limit_size;
    if (seq_len > 1){
        limit_size = size_per_head_div_x * head_num * seq_len;
    } else {
        limit_size = size_per_head_div_x * head_num * max_seq_len;
    }

    if (dst_idx >= limit_size) {
        return;
    }

    data_dst[dst_idx] = data_src[src_idx];
}

template<typename T>
__global__ void transpose_4d_kv_cache_2_kv(
    T* d_dst, T* d_src, const int head_num, const int size_per_head, const int cache_idx, const int max_seq_len)
{
    const int token_id = blockIdx.x;
    const int batch_id = blockIdx.y;
    const int head_id  = blockIdx.z;
    ...
    // 16 byte loads will handle "x" dimension
    auto data_src = reinterpret_cast<const uint4*>(d_src + (token_id * head_num * size_per_head) + (head_id * size_per_head));

    const int dst_index = batch_id * head_num * size_per_head + head_id * (cache_idx + 1) * size_per_head + token_id * size_per_head;
    auto data_dst = reinterpret_cast<uint4*>(d_dst + dst_index);

    const int trs_idx = threadIdx.x;

    data_dst[trs_idx] = data_src[trs_idx];
}
                                 ...
                                 cudaStream_t stream)
{
    ...
    constexpr int block_sz = 128 / (sizeof(uint4) / sizeof(T));
    ...
    dim3          grid_kv(seq_len, local_batch_size, local_head_num);

    transpose_4d_batch_major_kv_cache<<<grid_kv, block_sz, 0, stream>>>(
        k_dst, k_src, local_head_num, size_per_head, seq_len, cache_idx, max_seq_len);
    ...
    transpose_4d_batch_major_kv_cache<<<grid_kv, block_sz, 0, stream>>>(
        v_dst, v_src, local_head_num, size_per_head, seq_len, cache_idx, max_seq_len);

    if (seq_len == 1){
        dim3 grid_kv2((cache_idx + 1), local_batch_size, local_head_num);
        transpose_4d_kv_cache_2_kv<<<grid_kv2, block_sz, 0, stream>>>(
            k_src, k_dst, local_head_num, size_per_head, cache_idx, max_seq_len);
        transpose_4d_kv_cache_2_kv<<<grid_kv2, block_sz, 0, stream>>>(
            v_src, v_dst, local_head_num, size_per_head, cache_idx, max_seq_len);
    }
}
