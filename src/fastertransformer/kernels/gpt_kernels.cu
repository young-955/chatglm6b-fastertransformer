...
#pragma unroll 1
        ...
        // length = max_length:4, batch_size = batch_size:1 * beam_width:1 = 1, hidden_units_:4096, start_step=1
        ...
        const int step            = start_step + word_index_col;
                                                ...
                                                const int  max_prompt_length)
{
    ...
    for (int i = threadIdx.x; i < mask_size_per_seq; i += blockDim.x) {
        ...
        // }
        if (row_id < seq_length && (col_id == seq_length - 1 && row_id != col_id)) {
            attention_mask[i] = (T)(1.0f);
        }
        else {
            attention_mask[i] = (T)(0.0f);
