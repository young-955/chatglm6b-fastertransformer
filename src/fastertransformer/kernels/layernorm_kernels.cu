...
// * Note that typename T is half2 or bfloat2 type
// bool IS_OUTPUT = False, bool IS_BIAS = false(input_ln), int RESIDUAL_NUM = 1, bool IS_BETA = True, int UNROLL_FACTOR = 4
template<typename T, bool IS_OUTPUT, bool IS_BIAS, int RESIDUAL_NUM, bool IS_BETA, int UNROLL_FACTOR>
...
#pragma unroll UNROLL_FACTOR
    for (int i = threadIdx.x; i < n; i += blockDim.x) {
        ...
        if (RESIDUAL_NUM == 1) {
            ...
            // val_1 += static_cast<float>(tmp.x);
            // val_2 += static_cast<float>(tmp.y);
            val_1 += static_cast<float>(tmp.x) * 7.483314773547883;
            val_2 += static_cast<float>(tmp.y) * 7.483314773547883;
        }
        else if (RESIDUAL_NUM == 2) {
            tmp    = ldg(&residual1[index]);
        ...
        // x2_sum += (val_1 + val_2) * (val_1 + val_2);
    }
    float sums[2];
    sums[0] = x_sum;
    sums[1] = x2_sum;
    ...
    if (threadIdx.x == 0) {
        ...
        // s_variance = rsqrtf((sums[1] - s_mean) * (sums[1] - s_mean) / n / 2 + layernorm_eps);
    }
    __syncthreads();
...
{
    if (bias != nullptr) {
        // std::cout << "bias is true" << std::endl;
        ...
        // std::cout << "bias is false" << std::endl;
...
template<typename T>
void chatglm_input_layernorm(T*           out,
                            const T*     input,
                            const T*     gamma,
                            const T*     beta,
                            const float  layernorm_eps,
                            ...
                            int          opt_version)
{
    ...
    if (n % 2 == 0 && (std::is_same<T, half>::value)
        ...
        // unroll_factor:4,opt_version:2,half_n: 2048,layernorm_eps: 1e-06, scale: 0
...
template void chatglm_input_layernorm(float*       out,
                                     const float* input,
                                     const float* gamma,
                                     const float* beta,
                                     const float  layernorm_eps,
                                     ...
                                     cudaStream_t stream,
                                     int          opt_version);
template void chatglm_input_layernorm(half*        out,
                                     const half*  input,
                                     const half*  gamma,
                                     const half*  beta,
                                     const float  layernorm_eps,
                                     ...
                                     int          opt_version);
#ifdef ENABLE_BF16
template void chatglm_input_layernorm(__nv_bfloat16*       out,
                                     const __nv_bfloat16* input,
                                     const __nv_bfloat16* gamma,
                                     const __nv_bfloat16* beta,
                                     const float          layernorm_eps,
                            ...
                            int          opt_version)
{
    ...
    if (n % 2 == 0 && (std::is_same<T, half>::value)
        ...
        int  half_n    = n / 2;
        int  half_n_32 = (half_n + 31) / 32 * 32;
        // std::cout << "half_n:" << half_n << ", half_n_32:" << half_n_32 << std::endl;
        // dim3 block(min(half_n_32, 512));
        dim3 block(min(half_n_32, 512));
        int  rolls_per_thread = half_n / block.x;
        int  unroll_factor    = 8;
...
//     dim3 grid(1);
//     dim3 block(512);
//     argMax<<<grid, block, 0, stream>>>(logits, out_id, cur_idx, n, (n + 511) / 512);
// }

// template<typename T>
// __global__ void argMax(T* logits, const int* out_id, const int n, const int cur_idx, const int step)
// {
//     __shared__ int   max_id[512];
//     __shared__ float max_val_thrd[512];
...
//     for (int i = tid; i < n; i++) {
//         int   lg_idx  = offset + i;
//         float lgt_val = static_cast<float>(logits[lg_idx]);
//         if (max_val < lgt_val){
//             max_id[tid] = lg_idx;
//             max_val_thrd[tid] = lgt_val;
//         }
//     }

//     __syncthreads();

//     float max_val2  = -1 * FLT_MAX;
//     out_id[cur_idx] = 0;
//     for (int j = 0; j < 512; j++) {
//         if (max_val2 < max_val_thrd[j]){
//             max_val2 = max_val_thrd[j];
//             out_id[cur_idx]  = max_id[j];
//         }
//     }
// }
