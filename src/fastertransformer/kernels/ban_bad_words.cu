...
template<typename T>
__global__ void argMax(T* logits, int* out_id, const int n, const int cur_idx, const int step_stride)
{
    __shared__ int   max_id[512];
    __shared__ float max_val_thrd[512];

    int   tid       = threadIdx.x;
    int   offset    = tid * step_stride;
    float max_val   = -3.402823466E+38F;

    for (int i = 0; i < step_stride; i++) {
        int   lg_idx  = offset + i;
    ...
    if (tid == 0){
        float max_val2  = -3.402823466E+38F;
        out_id[cur_idx] = 150005;
        for (int j = 0; j < 512; j++) {
                                        ...
                                        cudaStream_t stream)
{
    ...
    // dim3 block(512);
    dim3 block(1);
    argMax<<<grid, block, 0, stream>>>(logits, out_id, n, cur_idx, (n + 511) / 512);
}

template void invokeArgMax(half*        logits,
                                        int*         out_id,
                                        const int    cur_idx,
                                        const int    n,
                                        cudaStream_t stream);
#ifdef ENABLE_BF16
template void invokeArgMax(__nv_bfloat16* logits,
                                        int*         out_id,
                                        const int    cur_idx,
                                        const int    n,
                                        cudaStream_t stream);
#endif
template void invokeArgMax(float*       logits,
                                        int*         out_id,
                                        const int    cur_idx,
                                        const int    n,
                                        cudaStream_t stream);
