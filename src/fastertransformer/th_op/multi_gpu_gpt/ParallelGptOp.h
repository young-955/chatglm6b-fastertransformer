...
public:
        ...
        shared_contexts_ratio_(shared_contexts_ratio)
    {
        ...
        if (gpt_variant_params_.has_positional_encoding) {
            ...
            // pre_decoder_embedding_table -> word_embeddings
            gpt_weights_.pre_decoder_embedding_table = get_ptr<T>(weights_[12 * layer_num_ + 4 - weight_offset]);
        }
        else {
            weight_offset += 1;
        }
        ...
        gpt_weights_.post_decoder_embedding.kernel = get_ptr<T>(weights_[12 * layer_num_ + 5 - weight_offset]);

        // weight_offset = 7 - weight_offset;
        weight_offset = 6 - weight_offset;
        std::cout << "the offset is: " << weight_offset << std::endl;
                 ...
                 th::optional<int64_t>    return_cum_log_probs_opt) override
    {
        ...
        std::cout << "before load gemm request_output_len: " << request_output_len << std::endl;
        ...
        std::cout << "after load gemm request_output_len: " << request_output_len << std::endl;
