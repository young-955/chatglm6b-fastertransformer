...
void ParallelGptContextDecoder<T>::allocateBuffer(size_t batch_size, size_t seq_len, bool use_shared_contexts)
{
    ...
    normed_self_attn_output_ = reinterpret_cast<T*>(
        allocator_->reMalloc(normed_self_attn_output_, sizeof(T) * batch_size * seq_len * hidden_units_, false));;  // reuse the buffer
    ...
    const std::vector<ParallelGptDecoderLayerWeight<T>*>* gpt_decoder_layer_weight)
{
    ...
    // use to gen pos id, add by wuziyang
    const int  step_start              = input_tensors->getVal<int>("step_start", 0);
    const int  cur_step                = input_tensors->getVal<int>("cur_step", 0);
    const int  position_id             = step_start == 0 ? 0 : step_start - 2;
    const int  block_position_id       = cur_step == 0 ? 0 : cur_step - 3;
    ...
    // self_k_cache_size: 1,32,16,36,8,
    ...
    for (uint ite = 0; ite < iteration_num; ite++) {
        ...
        for (uint l = 0; l < num_layer_; l++) {
            ...
            // if (l == 0){
            //     int decoder_normed_input__size = 4096;
            //     __half decoder_normed_input__cpu[decoder_normed_input__size];
            //     cudaMemcpy(decoder_normed_input__cpu, decoder_normed_input_, decoder_normed_input__size * sizeof(__half), cudaMemcpyDeviceToHost);

            //     std::cout << "decoder_normed_input_ res:" << std::endl;
            //     for (int i = 0; i < 32; i++)
            //         std::cout << ", " << decoder_normed_input__cpu[i];
            ...
            TensorMap self_attention_input_tensors{
                ...
                {"position_id", Tensor{MEMORY_CPU, TYPE_INT32, {(size_t)1}, &position_id}},
                {"block_position_id", Tensor{MEMORY_CPU, TYPE_INT32, {(size_t)1}, &block_position_id}}
            };
            ...
            if (l == 0){
                int decoder_normed_input_cpusize = 2 * 4096;
                __half decoder_normedcpu[decoder_normed_input_cpusize];
                cudaMemcpy(decoder_normedcpu, decoder_normed_input_, decoder_normed_input_cpusize * sizeof(__half), cudaMemcpyDeviceToHost);
            }
            ...
            // if (l == 0){
            //     int qkv_size = 2 * 4096;
            //     __half self_attn_output__cpu[qkv_size];
            //     cudaMemcpy(self_attn_output__cpu, self_attn_output_, qkv_size * sizeof(__half), cudaMemcpyDeviceToHost);
            //     std::cout << "self_attn_output__cpu2 res:" << std::endl;
            //     for (int i = 0; i < 128; i++)
            //         std::cout << ", " << self_attn_output__cpu[i];
            //     std::cout << std::endl << std::endl;
            // }
            ...
            // has_adapters_ = False
            if (layernorm_type_ == LayerNormType::pre_layernorm) {
                // layernorm_eps_: 1e-05, h_token_num: 4, hidden_units_: 4096, const_cast<float*>(layer_weight->ffn_weights.intermediate_weight.scale): 0
                ...
                // if (l == 0){
                    // std::cout << "layernorm_eps_: " << layernorm_eps_ << ", h_token_num: " << h_token_num << ", int8_mode_:" << int8_mode_ << ", hidden_units_:" << hidden_units_ << std::endl << std::endl;
                    // int qkv_size = 2 * 4096;
                    // __half qkv_cpu[qkv_size];
                    // __half self_attn_output__cpu[qkv_size];
                    // cudaMemcpy(qkv_cpu, normed_self_attn_output_, qkv_size * sizeof(__half), cudaMemcpyDeviceToHost);

                    // std::cout << "normed_self_attn_output_ res:" << std::endl;
                    // for (int i = 0; i < 128; i++)
                    //     std::cout << ", " << qkv_cpu[i];
                    // std::cout << std::endl << std::endl;
                    ...
                    // std::cout << "decoder_normedcpu res:" << std::endl;
                    // for (int i = 0; i < 128; i++)
                    //     std::cout << ", " << decoder_normedcpu[i];
                    // std::cout << std::endl << std::endl;
                // }
            }
            ...
            //     int qkv_size = 2 * 4096;
            //     __half qkv_cpu[qkv_size];
            //     cudaMemcpy(qkv_cpu, decoder_output, qkv_size * sizeof(__half), cudaMemcpyDeviceToHost);

            //     std::cout << "ffn_layer_ res:" << std::endl;
            //     for (int i = 0; i < 128; i++)
            //         std::cout << ", " << qkv_cpu[i];
            //     std::cout << std::endl << std::endl;
            ...
            if (!use_moe) {
                if (layernorm_type_ == LayerNormType::pre_layernorm) {
                    ...
                    //     std::cout << "invokeAddBiasResidual res:" << std::endl;
                    //     for (int i = 0; i < 128; i++)
                    //         std::cout << ", " << qkv_cpu[i];
                    //     std::cout << std::endl << std::endl;

                    //     for (int i = 0; i < 128; i++)
                    //         std::cout << ", " << qkv_cpu[i + 32 * 4];
                    //     std::cout << std::endl << std::endl;
