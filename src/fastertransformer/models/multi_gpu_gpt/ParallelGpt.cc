                                               ...
                                               const ParallelGptWeight<T>* gpt_weights)
{
    ...
    if (pipeline_para_.rank_ == pipeline_para_.world_size_ - 1) {
        ...
        if (tensor_para_.world_size_ == 1) {
            float alpha = 1.0f;
            float beta  = 0.0f;
            __half alphah = static_cast<__half>(1.0f);
            __half betah  = static_cast<__half>(0.0f);
                             ...
                             const ParallelGptWeight<T>*                    gpt_weights)
{
    ...
    if (continue_gen) {
        // std::cout << "continue gen is true.." << std::endl;
        ...
        // std::cout << "continue gen is false.." << std::endl;
        ...
        if (vocab_size_ == vocab_size_padded_) {
            // std::cout << "here embedding == lm_head" << std::endl;
        ...
        if (use_loaded_p_prompt_embedding) { // False
            // std::cout << "use_loaded_p_prompt_embedding" << std::endl;
            ...
            std::cout << "no use_loaded_p_prompt_embedding" << std::endl;
        }
        POP_RANGE;

        // handle first step
        if (has_p_prompt_tuning_ || has_prefix_prompt_ || has_prefix_soft_prompt_ || max_input_length > 1) {
            // std::cout << "now handle first step..." << std::endl;
            // std::cout << "use_request_p_prompt_embedding: " << use_request_p_prompt_embedding << std::endl;
            PUSH_RANGE("input tiling and init");
            ...
            if (has_prefix_soft_prompt_) {
                // std::cout << "pre_decoder_embedding_table 1 act!" << std::endl;
                ...
                // std::cout << "pre_decoder_embedding_table 2 act!" << std::endl;
                // NOTE: add prompt embeddings here (for p/prompt tuning)
                PUSH_RANGE("input id embedding lookup");
                pPromptTuningParam<T> prompt_param{
                    use_loaded_p_prompt_embedding ? prompt_learning_weight_batch_ : (const T**)nullptr, // false
                    prompt_learning_start_id_,
                    max_request_p_prompt_length,
                    use_request_p_prompt_embedding, // false
                    use_request_p_prompt_embedding ? input_tensors->at("request_prompt_embedding").getPtr<T>() :
                                                     nullptr}; // false
                // "prompt_learning_start_id_: 150006, max_request_p_prompt_length: 0"
                // std::cout << "max_input_length:" << max_input_length << ", batch_size:" << batch_size << ", beam_width:" << beam_width << ", hidden_units_:" << hidden_units_ << std::endl;
                
                int input_cpu[max_input_length];
                cudaMemcpy(input_cpu, tiled_input_ids_buf_, max_input_length * sizeof(int), cudaMemcpyDeviceToHost);

                //  20005, 94874, 150001, 150004
                // std::cout << "input id:";
            ...
            if (gpt_variant_params_.has_pre_decoder_layernorm) { // false
            ...
            int input_size = 1;
            int input_cpu[input_size];
            cudaMemcpy(input_cpu, tiled_input_lengths_buf_, input_size * sizeof(int), cudaMemcpyDeviceToHost);
    ...
    // step_start: 4, gen_len: 36, iteration_num:1
    for (step_ = step_start; step_ < (int)gen_len; step_++) {
        ...
        *generation_should_stop_   = !fill_caches_only;
        ...
        for (uint ite = 0; ite < iteration_num; ++ite) {
            ...
            int decoder_input_buf__size = 2 * 4096;
            __half decoder_input_buf__size_cpu[decoder_input_buf__size];
            __half cont_dec_cpu[decoder_input_buf__size];
            __half dec_cpu[decoder_input_buf__size];
            int outid_cpu[decoder_input_buf__size];
            // cudaMemcpy(decoder_input_buf__size_cpu, decoder_input_buf_, decoder_input_buf__size * sizeof(__half), cudaMemcpyDeviceToHost);
            // cudaMemcpy(cont_dec_cpu, context_decoder_output_buf_, decoder_input_buf__size * sizeof(__half), cudaMemcpyDeviceToHost);
            // cudaMemcpy(dec_cpu, decoder_output_buf_, decoder_input_buf__size * sizeof(__half), cudaMemcpyDeviceToHost);
            cudaMemcpy(outid_cpu, output_ids_buf_, decoder_input_buf__size * sizeof(int), cudaMemcpyDeviceToHost);

            // std::cout << "decoder_input_buf__size_cpu res:" << std::endl;
            // for (int i = 0; i < 4; i++)
            //     std::cout << ", " << decoder_input_buf__size_cpu[i];
            // std::cout << std::endl << std::endl;
            // std::cout << "cont_dec_cpu res:" << std::endl;
            // for (int i = 0; i < 4; i++)
            //     std::cout << ", " << cont_dec_cpu[i];
            // std::cout << std::endl << std::endl;
            // std::cout << "dec_cpu res:" << std::endl;
            // for (int i = 0; i < 4; i++)
            //     std::cout << ", " << dec_cpu[i];
            // std::cout << std::endl << std::endl;
            // std::cout << "outid_cpu res:" << std::endl;
            // for (int i = 0; i < (int)gen_len; i++)
            //     std::cout << ", " << outid_cpu[i];
            // std::cout << std::endl << std::endl;
            ...
            if ((max_input_length <= 1) || (step_ > step_start) || continue_gen) {
                ...
                TensorMap decoder_input_tensors(
                    ...
                    Tensor(MEMORY_GPU,
                            data_type,
                            // (1, 4096)
                            {batch_size * beam_width, 1, hidden_units_},
                            gpt_variant_params_.has_pre_decoder_layernorm ? decoder_normed_input_buf_ + hidden_units_offset :
                                                                            decoder_input_buf_ + hidden_units_offset)},
                    {"attention_mask",
                    Tensor(MEMORY_GPU,
                            data_type,
                    ...
                    {"step_start",
                    Tensor(MEMORY_CPU, TYPE_INT32, {(size_t)1}, &step_start)},

                    {"cur_step",
                    Tensor(MEMORY_CPU, TYPE_INT32, {(size_t)1}, &step_)}}
                );
            ...
            if (!fill_caches_only && pipeline_para_.rank_ == pipeline_para_.world_size_ - 1) {
                ...
                T* decoder_output_final_buf = normed_decoder_output_buf_;

                if (gpt_variant_params_.has_post_decoder_layernorm) {
                    invokeGeneralLayerNorm(normed_decoder_output_buf_ + hidden_units_offset,
                                            (step_ > step_start) ? context_decoder_output_buf_ + hidden_units_offset : decoder_output_buf_ + hidden_units_offset,
                                        gpt_weights->post_decoder_layernorm.gamma,
                                        gpt_weights->post_decoder_layernorm.beta,
                                        layernorm_eps_,
                                        local_batch_size * beam_width,
                ...
                if (tensor_para_.world_size_ == 1) {
                    ...
                    int usize = 20 * 4096;
                    __half out_buf_cpu[usize];
                    __half norm_decode_cpu[usize];
                    // cudaMemcpy(out_buf_cpu, decoder_output_buf_, usize * sizeof(__half), cudaMemcpyDeviceToHost);
                    // cudaMemcpy(ctxt_out_buf_cpu, context_decoder_output_buf_, usize * sizeof(__half), cudaMemcpyDeviceToHost);
                    // cudaMemcpy(ctxt_out_buf_cpu, logits_buf_, usize * sizeof(__half), cudaMemcpyDeviceToHost);
                    // cudaMemcpy(norm_decode_cpu, normed_decoder_output_buf_, usize * sizeof(__half), cudaMemcpyDeviceToHost);

                    // cudaMalloc((float**)&logit_gpu, vocab_size_padded_ * sizeof(float));

                    // for (int i = 0; i < 128; i++)
                    //     std::cout << ", " << norm_decode_cpu[i];
                    // std::cout << std::endl << std::endl;
                    // for (int i = 0; i < 128; i++)
                    //     std::cout << ", " << ctxt_out_buf_cpu[i];
                    // std::cout << std::endl << std::endl;
                    // for (int i = 0; i < 128; i++)
                    //     std::cout << ", " << ctxt_out_buf_cpu[i + 10000];
                    // std::cout << std::endl << std::endl;
                    // for (int i = 0; i < 128; i++)
                    //     std::cout << ", " << ctxt_out_buf_cpu[i + 53500];
                    // std::cout << std::endl << std::endl;
                    // for (int i = 0; i < 128; i++)
                    //     std::cout << ", " << ctxt_out_buf_cpu[i + 20000];
                    // std::cout << std::endl << std::endl;
                ...
                __half logit_buf_cpu[vocab_size_padded_];
                int output_id_cpu[gen_len];
                cudaMemcpy(logit_buf_cpu, logits_buf_, vocab_size_padded_ * sizeof(__half), cudaMemcpyDeviceToHost);
                cudaMemcpy(output_id_cpu, output_ids_buf_, gen_len * sizeof(int), cudaMemcpyDeviceToHost);
                float max_val= -33333333;
                out_id = 150005;
                for (int i = 0; i < vocab_size_padded_; i++) {
                    float lgt_val = static_cast<float>(logit_buf_cpu[i]);
                    if (max_val < lgt_val){
                        out_id  = i;
                        max_val = lgt_val;
                    }
                ...
                *generation_should_stop_ &= subbatch_should_stop;
                ...
                output_id_cpu[step_] = out_id;
                cudaMemcpy(output_ids_buf_, output_id_cpu, gen_len * sizeof(int), cudaMemcpyHostToDevice);
                // invokeArgMax(logits_buf_, output_ids_buf_, step_, vocab_size_padded_, stream_);
                // dynamic_decode_layer_->argmax(logits_buf_, output_ids_buf_, step_, vocab_size_padded_, stream_);                
            }
