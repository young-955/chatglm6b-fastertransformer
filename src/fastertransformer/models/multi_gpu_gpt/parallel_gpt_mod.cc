                             ...
                             const ParallelGptWeight<T>*                    gpt_weights)
{
    ...
    std::cout << "here forward start.." << std::endl;
    ...
    const int* prompt_learning_task_name_ids =
        input_tensors->count("prompt_learning_task_name_ids") ?
            ...
            nullptr;
    std::cout << "here 4 start.." << std::endl;
    ...
    std::cout << "here 5 start.." << std::endl;
    ...
    std::cout << "here 6 start.." << std::endl;
    ...
    std::cout << "here 7 start.." << std::endl;
    ...
    const uint32_t* input_lengths_h = input_tensors->count("input_lengths_h") ?
                                          ...
                                          nullptr;
    std::cout << "here 8 start.." << std::endl;
    ...
    std::cout << "here 9 start.." << std::endl;
    ...
    std::cout << "here 10 start.." << std::endl;
    ...
    std::cout << "here 11 start.." << std::endl;
    ...
    std::cout << "k cache shape: ";
    for (size_t ele: self_k_cache_shape){
        std::cout << ele << ",";
    }
    std::cout << std::endl;

    std::cout << "v cache shape: ";
    for (size_t ele: self_v_cache_shape){
        std::cout << ele << ",";
    }
    std::cout << std::endl;
    ...
    std::cout << "here 18 start.." << std::endl;
    ...
    invokeMaskPaddingTokens(masked_tokens_,
                            ...
                            stream_);
    POP_RANGE;
    std::cout << "here 19 start.." << std::endl;
    ...
    std::cout << "here 20 start.." << std::endl;
    ...
    for (step_ = step_start; step_ < (int)gen_len; step_++) {
        ...
        *generation_should_stop_   = !fill_caches_only;
        ...
        for (uint ite = 0; ite < iteration_num; ++ite) {
            ...
            std::cout << "gen_len: " << gen_len << ", iteration_num: " << iteration_num << ", id_offset: " << id_offset << ", local_batch_size: " << local_batch_size << ", beam_width: " << beam_width << ", ite: " << ite << ", hidden_units_offset: " << hidden_units_offset << ", vocab_size_units_offset: " << vocab_size_units_offset << ", hidden_units_: " << hidden_units_ << ", vocab_size_padded_: " << vocab_size_padded_ << std::endl;

            std::cout << "max_input_length:" << max_input_length << ", step_: " << step_ << ", step_start: " << step_start << ", continue_gen: " << continue_gen << ", output_ids_buf_:" << output_ids_buf_ << ", tiled_total_padding_count_: " << tiled_total_padding_count_ << std::endl;

            if ((max_input_length <= 1) || (step_ > step_start) || continue_gen) {
                if (pipeline_para_.rank_ == 0) {


                    std::cout << "pre_decoder_embedding_table 3 act!" << std::endl;
                    invokeEmbeddingLookupPosEncodingPadCount(decoder_input_buf_ + hidden_units_offset,
                                                             ...
                                                             stream_);
                    sync_check_cuda_error();
                // std::cout << "decoder_input_buf_: " << (float*)decoder_input_buf_ + 1 << std::endl;
                std::cout << "then the loop 1-7 start.." << std::endl;

                    if (gpt_variant_params_.has_pre_decoder_layernorm) {
                        std::cout << "here invokeGeneralLayerNorm(decoder_normed_input_buf_" << std::endl;
            ...
            if (!fill_caches_only && pipeline_para_.rank_ == pipeline_para_.world_size_ - 1) {
                std::cout << "first should jump in here!" << std::endl;
                // OPT
                ...
                if (gpt_variant_params_.has_post_decoder_layernorm) {
                    invokeGeneralLayerNorm(normed_decoder_output_buf_ + hidden_units_offset,
                                            decoder_output_buf_ + hidden_units_offset,
                                        //    (step_ > step_start) ? decoder_output_buf_ + hidden_units_offset : context_decoder_output_buf_ + hidden_units_offset,
                                           gpt_weights->post_decoder_layernorm.gamma,
                                           gpt_weights->post_decoder_layernorm.beta,
                                           layernorm_eps_,
                                           local_batch_size * beam_width,
                ...
                if (tensor_para_.world_size_ == 1) {
                    std::cout << "then the loop 1-13 start.." << std::endl;
                    float alpha = 1.0f;
                    float beta  = 0.0f;
                    PUSH_RANGE("logits gemm");
                    std::cout << "m:" << vocab_size_padded_ << ", n: " << local_batch_size * beam_width << ", k: " << hidden_units_ << ", hidden_units_offset:" << hidden_units_offset << ", vocab_size_units_offset:" << vocab_size_units_offset << std::endl;
                    // std::cout << "decoder_output_final_buf: " << decoder_output_final_buf << ", padded_embedding_kernel_ptr_: " << padded_embedding_kernel_ptr_ << std::endl;
                    // m:150528, n: 1, k: 4096
                    // m * k X k * n
                    cublas_wrapper_->Gemm(CUBLAS_OP_T,
                                          CUBLAS_OP_N,
                ...
                // std::cout << "ite:" << *(int*)ite << std::endl;
                // std::cout << "end_id:" << *(int*)end_ids_buf_ << std::endl;
                // float* tmp;
                // *tmp = logits_buf_[0];
                // printf("logits_buf_:%s", *tmp );
                // std::cout << Tensor{MEMORY_CPU, TYPE_INT32, {1}, &step_}.toString() << std::endl;
                // std::cout << "step data:" << *(&step_) << std::endl;
                // std::cout << "logits_buf_:" << *(&logits_buf_) << std::endl;
                // std::cout << "logits_buf_:" << logits_buf_ << std::endl;
                // printf("logits_buf_: %s", *logits_buf_);
                ...
                for (auto t = input_tensors->begin(); t != input_tensors->end(); ++t) {
                    printf("input123456789: %s", t->first);
                ...
                for (auto t = output_tensors->begin(); t != output_tensors->end(); ++t) {
                    // Handle exceptions.
                    printf("output123456789: %s", t->first);
        ...
        std::cout << "then the sec 2 start.." << std::endl;
        ...
        // std::cout << "then the sec 3 start.." << std::endl;
