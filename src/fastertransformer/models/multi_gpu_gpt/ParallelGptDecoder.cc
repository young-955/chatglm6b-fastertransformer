                                    ...
                                    const std::vector<ParallelGptDecoderLayerWeight<T>*>* gpt_decoder_layer_weight)
{
    ...
    for (uint l = 0; l < num_layer_; l++) {
        ...
        // if (l == 0){
        //     int decoder_inpubuf__size = 2 * 4096;
        //     __half decoder_input_buf__size_cpu[decoder_inpubuf__size];
        //     __half decoder_normed_input__cpu[decoder_inpubuf__size];
        //     cudaMemcpy(decoder_input_buf__size_cpu, decoder_input, decoder_inpubuf__size * sizeof(__half), cudaMemcpyDeviceToHost);
        //     cudaMemcpy(decoder_normed_input__cpu, decoder_normed_input_, decoder_inpubuf__size * sizeof(__half), cudaMemcpyDeviceToHost);

        //     std::cout << "decoder_input res:" << std::endl;
        //     for (int i = 0; i < 4; i++)
        ...
        T* ffn_output_ptr = decoder_output;
        ...
        if (!use_moe) {
            if (has_adapters_) {std::cout << " what a fuck thing!!!!!!!!!!!" << std::endl;}
            if (layernorm_type_ == LayerNormType::pre_layernorm) {
            ...
            std::cout << "here is worse!!" << std::endl;
            if (layernorm_type_ == LayerNormType::pre_layernorm) {
