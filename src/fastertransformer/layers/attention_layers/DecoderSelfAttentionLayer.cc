                                        ...
                                        cudaStream_t stream)
{
    ...
    // params.timestep 0~n, params.hidden_size_per_head 128
    // std::cout << "now decode params. timestep:" << params.timestep << ", hidden size per head:" << params.hidden_size_per_head << std::endl;
...
{
    printf("here to print the value dim: %d \r\n", attention_layer.rotary_embedding_dim_);
}
                                           ...
                                           const AttentionWeight<T>* attention_weights)
{
    ...
    const int  layer_id = *input_tensors->getPtr<int>("layer_id");
    ...
    if (print_flag){
        ...
        cudaMemcpy(inp_cpu, attention_input, attention_size * sizeof(__half), cudaMemcpyDeviceToHost);
        ...
        std::cout << "qkv_buf_ res:" << std::endl;
        for (int i = 0; i < 32; i++)
            std::cout << ", " << qkv_cpu[i];
        std::cout << std::endl << std::endl;
    }
    ...
    POP_RANGE;
    // printf("rotary_embedding_dim_ is : %d\r\n", rotary_embedding_dim_);
    // rotary_embedding_dim_ = 64
    ...
    if (print_flag){
        int context_buf__cpu_size2 = 4096;
        __half context_buf__cpu[context_buf__cpu_size2];
        __half kc_cpu[context_buf__cpu_size2];
        __half vc_cpu[context_buf__cpu_size2];
        cudaMemcpy(context_buf__cpu, context_buf_, context_buf__cpu_size2 * sizeof(__half), cudaMemcpyDeviceToHost);
        cudaMemcpy(kc_cpu, key_cache, context_buf__cpu_size2 * sizeof(__half), cudaMemcpyDeviceToHost);
        cudaMemcpy(vc_cpu, value_cache, context_buf__cpu_size2 * sizeof(__half), cudaMemcpyDeviceToHost);

        std::cout << "context_buf_ res:" << std::endl;
        for (int i = 0; i < 32; i++)
            std::cout << ", " << context_buf__cpu[i];
        std::cout << std::endl << std::endl;
        print_flag = false;
    }
