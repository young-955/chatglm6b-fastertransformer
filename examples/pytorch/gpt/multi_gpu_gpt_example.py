...
def main():
    ...
    parser.add_argument('--handset_cuda_device', type=int, default=-1,
                        help='handset cuda device')
    ...
    if args.use_jieba_tokenizer:
        ...
        enc = ChatGLMTokenizer.from_pretrained(args.vocab_file, trust_remote_code=True)
    torch.manual_seed(0)
    ...
    if not args.use_gpt_decoder_ops:
        gpt = ParallelGPT(head_num, size_per_head, vocab_size, start_id, end_id,
                          ...
                          handset_cuda_device=args.handset_cuda_device,
                          )
                        #   has_positional_encoding=False)        
        print('parallel model loaded ')
        if not gpt.load(ckpt_path=args.ckpt_path):
            print("[WARNING] Checkpoint file not found. Model loading is skipped.")
    else:
        print('gpt decoder model ')
    ...
    print(f'infer_decode_args: {infer_decode_args}')
    print(f'gpt layernorm type: {gpt.layernorm_type}, activate type: {gpt.activation_type}')

    if not args.use_gpt_decoder_ops:
        print('not use_gpt_decoder_ops ')
        def gpt_generate_fn():
    ...
    # cold start
    gpt_generate_fn()
    print(gpt.weights.w[0])
    print(f'start_lengths: {start_lengths}, start_ids: {start_ids}, shape: {start_ids.shape}')
    import time
    t1 = time.time()
    ...
    t2 = time.time()
    os.system('nvidia-smi')
    print(f'generate1 cost: {t2-t1}')
    # gen_outputs = gpt_generate_fn()
    # t3 = time.time()
    # print(f'generate2 cost: {t3-t2}')

    print(f'rank: {rank}')
    print(f'type: {type(gen_outputs)}, ')
    if rank == 0 or rank== -1:
        if not args.use_gpt_decoder_ops:
