...
# str_type_map = {"fp32": torch.float32, "fp16": torch.half, "bf16": torch.bfloat16}
...
class GPTWeights:
    def __init__(self, head_num, size_per_head, layer_num, vocab_size, max_seq_len, tensor_para_size, pipeline_para_size,
                 ...
                 int8_mode: int = 0,
                 inter_size: int = 0):
        print(f'{head_num}, {size_per_head}, {layer_num}, {vocab_size}, {max_seq_len}, {tensor_para_size}, {pipeline_para_size}, {weights_data_type}, {inference_data_type}, {has_adapters}, {adapter_inter_size}, {gpt_with_moe}, {has_positional_encoding}, {has_pre_decoder_layernorm}, {has_post_decoder_layernorm}, {int8_mode}, {inter_size}')
        t1 = time.time()
        ...
        t2 = time.time()
        print(f'before init weight cost: {t2 - t1}')
        # Transformer blocks
        ...
        self.w.append(torch.zeros(global_hidden_units, vocab_size,
        ...
        self.adapter_offset = 1 + optional_adapter_offset
        ...
        t3 = time.time()
        print(f'init weight cost: {t3 -t2}')
        # Initialization
#        self._map(lambda w: torch.nn.init.normal_(w, mean=0., std=1.))
        ...
        print(f'last init cost: {time.time() - t3}')
    ...
    def load(self, ckpt_path, tp_rank, pipeline_para_rank):
        ...
        w.extend([load_to_torch(f"{ckpt_path}/model.transformer.layers.{i}.input_layernorm.weight.bin", is_load(i))
                 for i in range(self.layer_num)])
        w.extend([load_to_torch(f"{ckpt_path}/model.transformer.layers.{i}.input_layernorm.bias.bin", is_load(i))
                 for i in range(self.layer_num)])

        w.extend([load_to_torch(
            f"{ckpt_path}/model.transformer.layers.{i}.attention.query_key_value.weight.bin", is_load(i)) for i in range(self.layer_num)])
            # f"{ckpt_path}/model.transformer.layers.{i}.attention.query_key_value.weight.{tp_rank}.bin", is_load(i)) for i in range(self.layer_num)])
        w.extend([load_to_torch(
            f"{ckpt_path}/model.transformer.layers.{i}.attention.query_key_value.bias.bin", is_load(i)) for i in range(self.layer_num)])
            # f"{ckpt_path}/model.transformer.layers.{i}.attention.query_key_value.bias.{tp_rank}.bin", is_load(i)) for i in range(self.layer_num)])

        w.extend([load_to_torch(f"{ckpt_path}/model.transformer.layers.{i}.attention.dense.weight.{tp_rank}.bin",
                 is_load(i)) for i in range(self.layer_num)])
        w.extend([load_to_torch(f"{ckpt_path}/model.transformer.layers.{i}.attention.dense.bias.bin", is_load(i))
                 for i in range(self.layer_num)])

        w.extend([load_to_torch(f"{ckpt_path}/model.transformer.layers.{i}.post_attention_layernorm.weight.bin",
                 is_load(i)) for i in range(self.layer_num)])
        w.extend([load_to_torch(f"{ckpt_path}/model.transformer.layers.{i}.post_attention_layernorm.bias.bin",
                 is_load(i)) for i in range(self.layer_num)])

        w.extend([load_to_torch(
                f"{ckpt_path}/model.transformer.layers.{i}.mlp.dense_h_to_4h.weight.{tp_rank}.bin" \
                    if os.path.isfile(f"{ckpt_path}/model.transformer.layers.{i}.mlp.dense_h_to_4h.weight.{tp_rank}.bin") \
                        else f"{ckpt_path}/model.transformer.layers.{i}.mlp.moe.experts.dense_h_to_4h.weight.{tp_rank}.bin",
                is_load(i)) for i in range(self.layer_num)])
        w.extend([load_to_torch(
                f"{ckpt_path}/model.transformer.layers.{i}.mlp.dense_h_to_4h.bias.{tp_rank}.bin" \
                    if os.path.isfile(f"{ckpt_path}/model.transformer.layers.{i}.mlp.dense_h_to_4h.bias.{tp_rank}.bin") \
                        else f"{ckpt_path}/model.transformer.layers.{i}.mlp.moe.experts.dense_h_to_4h.bias.{tp_rank}.bin",
                is_load(i)) for i in range(self.layer_num)])
        w.extend([load_to_torch(
                f"{ckpt_path}/model.transformer.layers.{i}.mlp.dense_4h_to_h.weight.{tp_rank}.bin" \
                    if os.path.isfile(f"{ckpt_path}/model.transformer.layers.{i}.mlp.dense_4h_to_h.weight.{tp_rank}.bin") \
                        else f"{ckpt_path}/model.transformer.layers.{i}.mlp.moe.experts.dense_4h_to_h.weight.{tp_rank}.bin",
                is_load(i)) for i in range(self.layer_num)])
        w.extend([load_to_torch(
                f"{ckpt_path}/model.transformer.layers.{i}.mlp.dense_4h_to_h.bias.bin" \
                    if os.path.isfile(f"{ckpt_path}/model.transformer.layers.{i}.mlp.dense_4h_to_h.bias.bin") \
                        else f"{ckpt_path}/model.transformer.layers.{i}.mlp.moe.experts.dense_4h_to_h.bias.bin",
                is_load(i)) for i in range(self.layer_num)])
        ...
        if self.has_positional_encoding:
            print('need position layer')

        w.append(load_to_torch(f"{ckpt_path}/model.word_embedding.bin", True))
        ...
        # gate_list = []
        # for i in range(self.layer_num):
        #     if (os.path.isfile(f"{ckpt_path}/model.transformer.layers.{i}.mlp.moe.gate.wg.weight.bin")):
        #         gate_list.append(load_to_torch(f"{ckpt_path}/model.transformer.layers.{i}.mlp.moe.gate.wg.weight.bin", True))
        #     else:
        #         gate_list.append(load_to_torch(f"{ckpt_path}/model.transformer.layers.{i}.mlp.moe.gate.wg.weight.bin", False))
        ...
        if self.has_adapters:
            w.extend([load_to_torch(
                    f"{ckpt_path}/model.transformer.layers.{i}.after_attention_adapter.dense_h_to_4h.weight.{tp_rank}.bin" \
                        if os.path.isfile(f"{ckpt_path}/model.transformer.layers.{i}.after_attention_adapter.dense_h_to_4h.weight.{tp_rank}.bin") \
                            else f"{ckpt_path}/model.transformer.layers.{i}.after_attention_adapter.moe.experts.dense_h_to_4h.weight.{tp_rank}.bin",
                    is_load(i)) for i in range(self.layer_num)])
            w.extend([load_to_torch(
                    f"{ckpt_path}/model.transformer.layers.{i}.after_attention_adapter.dense_h_to_4h.bias.{tp_rank}.bin" \
                        if os.path.isfile(f"{ckpt_path}/model.transformer.layers.{i}.after_attention_adapter.dense_h_to_4h.bias.{tp_rank}.bin") \
                            else f"{ckpt_path}/model.transformer.layers.{i}.after_attention_adapter.moe.experts.dense_h_to_4h.bias.{tp_rank}.bin",
                    is_load(i)) for i in range(self.layer_num)])
            w.extend([load_to_torch(
                    f"{ckpt_path}/model.transformer.layers.{i}.after_attention_adapter.dense_4h_to_h.weight.{tp_rank}.bin" \
                        if os.path.isfile(f"{ckpt_path}/model.transformer.layers.{i}.after_attention_adapter.dense_4h_to_h.weight.{tp_rank}.bin") \
                            else f"{ckpt_path}/model.transformer.layers.{i}.after_attention_adapter.moe.experts.dense_4h_to_h.weight.{tp_rank}.bin",
                    is_load(i)) for i in range(self.layer_num)])
            w.extend([load_to_torch(
                    f"{ckpt_path}/model.transformer.layers.{i}.after_attention_adapter.dense_4h_to_h.bias.bin" \
                        if os.path.isfile(f"{ckpt_path}/model.transformer.layers.{i}.after_attention_adapter.dense_4h_to_h.bias.bin") \
                            else f"{ckpt_path}/model.transformer.layers.{i}.after_attention_adapter.moe.experts.dense_4h_to_h.bias.bin",
                    is_load(i)) for i in range(self.layer_num)])
            w.extend([load_to_torch(
                    f"{ckpt_path}/model.transformer.layers.{i}.after_ffn_adapter.dense_h_to_4h.weight.{tp_rank}.bin" \
                        if os.path.isfile(f"{ckpt_path}/model.transformer.layers.{i}.after_ffn_adapter.dense_h_to_4h.weight.{tp_rank}.bin") \
                            else f"{ckpt_path}/model.transformer.layers.{i}.after_ffn_adapter.moe.experts.dense_h_to_4h.weight.{tp_rank}.bin",
                    is_load(i)) for i in range(self.layer_num)])
            w.extend([load_to_torch(
                    f"{ckpt_path}/model.transformer.layers.{i}.after_ffn_adapter.dense_h_to_4h.bias.{tp_rank}.bin" \
                        if os.path.isfile(f"{ckpt_path}/model.transformer.layers.{i}.after_ffn_adapter.dense_h_to_4h.bias.{tp_rank}.bin") \
                            else f"{ckpt_path}/model.transformer.layers.{i}.after_ffn_adapter.moe.experts.dense_h_to_4h.bias.{tp_rank}.bin",
                    is_load(i)) for i in range(self.layer_num)])
            w.extend([load_to_torch(
                    f"{ckpt_path}/model.transformer.layers.{i}.after_ffn_adapter.dense_4h_to_h.weight.{tp_rank}.bin" \
                        if os.path.isfile(f"{ckpt_path}/model.transformer.layers.{i}.after_ffn_adapter.dense_4h_to_h.weight.{tp_rank}.bin") \
                            else f"{ckpt_path}/model.transformer.layers.{i}.after_ffn_adapter.moe.experts.dense_4h_to_h.weight.{tp_rank}.bin",
                    is_load(i)) for i in range(self.layer_num)])
            w.extend([load_to_torch(
                    f"{ckpt_path}/model.transformer.layers.{i}.after_ffn_adapter.dense_4h_to_h.bias.bin" \
                        if os.path.isfile(f"{ckpt_path}/model.transformer.layers.{i}.after_ffn_adapter.dense_4h_to_h.bias.bin") \
                            else f"{ckpt_path}/model.transformer.layers.{i}.after_ffn_adapter.moe.experts.dense_4h_to_h.bias.bin",
                    is_load(i)) for i in range(self.layer_num)])

        print(f'selfw {len(self.w)}, w, {len(w)}')
...
class GPT(nn.Module):
    def __init__(self,
        ...
        print('load lib finish')
        # print('now load trans lib')
        # '''
        #     libmpich.so.12, 需要export mpi的bin路径到LD_LIBRARY_PATH
        # '''
        # transformer_lib_path = '/mnt/chatglm-6b/fastertransformer_model/Fasttsfm/FasterTransformer/lib/libth_transformer.so'
        # torch.classes.load_library(os.path.abspath(transformer_lib_path))
        # print('load lib trans finish')
        ...
        # try:
        res = dist.init_process_group(backend='mpi')
        print(f'multi process res: {res}')
        # except:
        #     print("[INFO] WARNING: Have initialized the process group")

        # print('now set a 600s delay to set ranks, please wait...')
        # import time
        # time.sleep(600)

        self.rank = dist.get_rank()
        self.device_count = torch.cuda.device_count()
        if handset_cuda_device == -1:
            self.device = self.rank % self.device_count
        else:
            self.device = handset_cuda_device
        torch.cuda.set_device(self.device)
        print(f'rank: {self.rank}, device_count: {self.device_count}')

        # world_size = dist.get_world_size()
        # print(f'world_size: {world_size}, tensor_para_size: {tensor_para_size}, pipeline_para_size: {pipeline_para_size}')
        ...
        self.tensor_para_rank = 0
        self.pipeline_para_rank = 0
